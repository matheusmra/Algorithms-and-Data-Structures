...
public class Q11 {
    ...
    /**
     * Counting Sort levando em consideração o ano
     */
    public static void countingSort(Show[] array, int n) {
        if (n == 0) return;

        // Encontra o menor e maior ano de lançamento para definir o intervalo
        ...
        int range = max - min + 1; // intervalo necessário
        int[] count = new int[range]; // vetor de contagem
        Show[] output = new Show[n];  // vetor de saída ordenado por ano

        // Conta quantas vezes cada ano aparece
        for (int i = 0; i < n; i++) {
            count[array[i].getReleaseYear() - min]++;
            mov++;
        }

        // Converte count[] para posições acumuladas
        for (int i = 1; i < range; i++) {
            count[i] += count[i - 1];
        }

        // Preenche o vetor de saída de forma estável
        for (int i = n - 1; i >= 0; i--) {
        ...
        // Desempata por título (ordem alfabética) para elementos com o mesmo ano
        desempatePorTitulo(array, n);
    }

    public static void desempatePorTitulo(Show[] array, int n) {
        for (int i = 1; i < n; i++) {
            Show temp = array[i];
            int j = i - 1;

            // Move elementos maiores que temp.title para frente
    ...
    public static void main(String[] args) throws Exception {
        ...
        Show[] base = Show.csv("TPS/02/tmp/disneyplus.csv");

        Show[] vetor = new Show[1400]; // Vetor de entrada do usuário
        int n = 0;                     // Tamanho do vetor final
        ...
        while (!entrada.equals("FIM")) {
            for (Show s : base) {
                if (s.getShowId().equals(entrada)) {
                    vetor[n++] = s; // Adiciona o show correspondente ao vetor
                    break;
                }
            }
        ...
        // Ordena vetor e mede tempo de execução
