...
// Insere na fila circular (remove se cheia, sem imprimir nada)
void inserirFila(FilaCircular* f, Show* show) {
    if (f->tamanho == TAM_FILA) {
        // Remove o mais antigo (sem imprimir)
        free(f->array[f->primeiro]);
...
// Remove do início (com impressão)
...
// Calcula a média truncada dos releaseYear na fila
int mediaReleaseYear(FilaCircular* f) {
    ...
    return soma / f->tamanho;
}

// Mostra todos os elementos da fila circular
