...
public class Q07 {
    public static void main(String[] args) throws Exception {
        ...
        insertionSort(vetor, n, comparacoes, movimentacoes);
        ...
        FileWriter log = new FileWriter(new File("matricula_insercao.txt"));
        log.write(matricula + "\t" + comparacoes + "\t" + movimentacoes + "\t" + tempo);
        log.close();
    }

    // Função para realizar o Insertion Sort
    public static void insertionSort(Show[] vetor, int n, int comparacoes, int movimentacoes) {
        for (int i = 1; i < n; i++) {
            Show chave = vetor[i];
            int j = i - 1;
            // Compara o tipo, se for igual, compara title
            while (j >= 0 && (vetor[j].getType().compareTo(chave.getType()) > 0 || 
                (vetor[j].getType().equals(chave.getType()) && vetor[j].getTitle().compareTo(chave.getTitle()) > 0))) {
