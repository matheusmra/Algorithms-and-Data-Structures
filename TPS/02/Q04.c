...
void quickSortByTitle(int esq, int dir, Show** array) {
    ...
    if (esq < j) quickSortByTitle(esq, j, array);
    if (i < dir) quickSortByTitle(i, dir, array);
}
...
int main(){
    ...
    quickSortByTitle(0, j - 1, ArrayShow);
    ...
    // Arquivo
    FILE* log = fopen("848813_binaria.txt", "w");
    if (log == NULL) {
        return 1;
    }
    fprintf(log, "848813\t");
